...
# Storing this in a function so I can easily change it to read from a text file later if needed
def load_api_key():
...
# Hardcoded list of tickers for the simulation to track
...
# Object-oriented programming: Stock class to hold individual stock data
class Stock:
    ...
    def fetch_current_price(self):
        ...
        try: # Using try/except for robustness so the program doesn't crash on a bad connection
    ...
    def fetch_historical(self):
        ...
        try:
            ...
            # Added this for testing purposes so my NEA doesn't break during the demo
            ...
            if data.get("s") == "ok":
                self.history = []
                # Loop through the API data arrays and match timestamps to closing prices
    ...
    def generate_mock_history(self):
        ...
        # Using a loop to populate the history array
        for i in range(30):
        ...
        if self.history:
            if len(self.history) > 1:
                if self.price > 0:
                    ...
                    if old_price != 0: # Prevent division by zero error
                        change = ((self.price - old_price) / old_price) * 100
                        return change
        return 0


# Portfolio class to handle user's money and bought stocks
class Portfolio:
    def __init__(self):
        self.cash = 100000.0 # Starting balance
        self.holdings = {} # Dictionary to store what stocks I own
        self.avg_price = {}
        self.realised_profit = 0.0
        self.history = []
        self.transaction_log = []

        # Load any saved data when the program starts
        self.load_from_json()
        self.load_history()
    ...
    def buy(self, stock, amount):
        ...
        # Validation checks before allowing the purchase
        if amount <= 0:
        ...
        # If I already own this stock, I need to recalculate the average price
        ...
        # Automatically save after buying
    ...
    def sell(self, stock, amount):
        ...
        # Clean up the dictionary if I sell everything
    ...
    # Calculates potential profit if everything was sold right now
    ...
    # Saving the main portfolio data to a JSON file
    def save(self):
        data = {
            "cash": self.cash,
...
# This creates a pop-up window for the portfolio summary
class PortfolioWindow(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        # Get the numbers for the labels
        ...
        # Creating the text labels for the top of the window
        ...
        # Using matplotlib to draw the graph of net worth over time
        ...
        # Embedding the matplotlib graph into tkinter
...
# Window to show the history of buys and sells
class TransactionWindow(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        # Using Treeview to make a nice table format
    ...
    # Fills the table with data from the JSON log
    def load_transactions(self):
        for row in self.table.get_children():
            self.table.delete(row)
        # Reverse the list so newest transactions are at the top
...
# Main application class for the core UI
class StockApp:
    def __init__(self, root):
        self.root = root
        self.stocks = {}
        # Setting up stock objects for every ticker in the list
        for sym in TICKERS:
        ...
        # Applying a nice dark theme for the UI components
        style = ttk.Style()
        ...
        # Control bar at the top for inputs and buttons
        ...
        # Binding commands to buttons so they run the right functions
        ...
        # Right table: My current holdings
        ...
        # Start pulling data and schedule the refresh loop
        self.update_data(initial_fetch=True)
        self.root.after(REFRESH_INTERVAL, self.auto_refresh)

    # Handles the logic when the Buy button is clicked
    def buy_stock(self):
    ...
    # Handles the logic when the Sell button is clicked
    def sell_stock(self):
        sym = self.stock_choice.get()
        try:
            amount = int(self.amount_entry.get())
    ...
    # Shows the individual chart for a specific stock
    def show_graph(self):
        sym = self.stock_choice.get()
    ...
    # Wipes all files to test from a clean slate
    ...
    # Loops through and calls the API for each stock
    def update_data(self, initial_fetch=False):
        for stock in self.stocks.values():
            stock.fetch_current_price()
            if not stock.history:
                stock.fetch_historical()
                # Sleep to avoid hitting the API rate limit too fast
                time.sleep(1.0)
        self.update_display()

    # Refreshes the numbers in the Treeview tables
    ...
    # Recursively calls itself to keep the prices live
